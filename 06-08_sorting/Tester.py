...
class Tester():
        ...
        for folder in list_of_folders:
            ...
            while True:
                ...
                if os.path.isfile(inFile) and os.path.isfile(outFile):
                    print(f'Test #{nr} :')
                    if not skip_run:
                        result, total_time, cmp, asg = self.run_test(inFile, outFile)
                        if isinstance(total_time, int): assert result == True
                    else:
                        result, total_time, cmp, asg = None, 'timeout', '--', '--'
                    print(f'Test result: {result}, time: {total_time} ms')
                    self.output[key_name][nr] = [total_time, cmp, asg]
                    if result is None:
                        skip_run = True
                    nr += 1
                    continue
    ...
    def run_test(self, inFile, outFile):
        ...
        try:
            ...
            return expect == sort.array, total_time, sort.cmp, sort.asg
